// lib/init-db.ts  (adds period_closes; everything else unchanged)
import sql from './db'
...
export async function initDb() {
  ...
  await sql`
    CREATE TABLE IF NOT EXISTS recurring_schedules (
      ...
      growth_pct DECIMAL(6,2) DEFAULT 0,
      start_date DATE,
      end_date DATE,
      status TEXT DEFAULT 'active',
      created_at TIMESTAMP DEFAULT NOW()
  ...
  // --- period locking (soft close): closed quarters don't regenerate forecasts ---
  await sql`
