// lib/init-db.ts  (adds transactions.status + recurring_schedules; everything else unchanged)
import sql from './db'
...
export async function initDb() {
  ...
  `
  // status flag: 'actual' (paid/recorded) vs 'forecast' (generated by the scheduler)
  ...
  // --- recurring schedules: generate forward forecast transactions on a cadence ---
  await sql`
    CREATE TABLE IF NOT EXISTS recurring_schedules (
      ...
      category TEXT NOT NULL,
      description TEXT,
      amount DECIMAL(12,2) NOT NULL,      -- amount to post each occurrence
      is_estimate BOOLEAN DEFAULT FALSE,  -- true = generated rows are estimates to confirm (insurance/tax)
      frequency TEXT NOT NULL,            -- 'monthly' | 'quarterly' | 'semiannual' | 'annual'
      months_csv TEXT,                    -- which months fire, e.g. '1,4,7,10'; blank = every month
      day_of_month INTEGER DEFAULT 15,
      growth_pct DECIMAL(6,2) DEFAULT 0,  -- reference only (e.g. tax escalation)
      start_date DATE,
      end_date DATE,
